...
const warnings = (source, modify) => {
    if (!source)
        return ui.notifications.warn(getSetting(modify.reason));
...
const createMessageOnModification = async (context, players) => context === 'gift' ? await createNewMessage(context, players[1], players[0]) : await createNewMessage(context, players[0]);
/**
...
const getControls = (players, index) => {
    ...
    if (game.user.isGM) {
        ...
        if (players.users[index].isGM)
            return [getSetting('nameOfBonusDieList')];
        else
